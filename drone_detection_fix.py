# Drone Detection Video Benchmark (Python Script)
# Research-grade frame-based benchmarking for YOLO drone-only detection on videos with per-frame GT.
# - No Jupyter widgets
...
# --- Paths ---
# Set these to your environment. Keep raw strings for Windows paths.
MODEL_WEIGHTS = r"S:\IntelliJ\Projects\ES_Drone_Detection\runs\detect\train7\weights\best.pt"
...
ALERTWIN_CASCADE_MIN_ACCEPTS = 9           # how many of the window frames must verify to accept alert
...
class TemporalContinuity:
    ...
    def expansion_factor(self):
        ...
        return float(clamp(2.0 + 0.15 * (self.stride - 1), 2.0, 4.0))
...
def main():
    ...
    while True:
        ...
        if is_infer:
            ...
            # Cap to max 3 predictions pre-postproc
            if len(final_boxes) > int(MAX_FULLFRAME_DETECTIONS):
            ...
            if val_post_mode == "Alert-Window Cascade":
                ...
                if alert_active_pre:
                    ...
                    total_infer_time_s += (t_verify1 - t_verify0)  # count as postproc time
            ...
            gt_warning_active = (len(gt_win_warning) == int(WARNING_WINDOW_FRAMES) and sum(gt_win_warning) >= int(WARNING_REQUIRE_HITS)) if gt_warn_cooldown_left <= 0 else False
            gt_alert_active = (len(gt_win_alert) == int(ALERT_WINDOW_FRAMES) and sum(gt_win_alert) >= int(ALERT_REQUIRE_HITS)) if gt_alert_cooldown_left <= 0 else False
        ...
        if val_show_verify:
            for roi in last_infer_verify_rois[:int(MAX_GUIDED_ROIS)]:
                draw_box(vis, roi, (0, 165, 255), 1, label="CascadeROI", anchor="br")
        ...
        if TOPLEFT_LOG_MODE != "off":
            if TOPLEFT_LOG_MODE == "windows_big":
                lines = [
                    f"WARNING  hits {sum(win_warning)}/{len(win_warning)}  need {WARNING_REQUIRE_HITS}/{WARNING_WINDOW_FRAMES}  cd={warn_cooldown_left}",
                    f"ALERT    hits {sum(win_alert)}/{len(win_alert)}  need {ALERT_REQUIRE_HITS}/{ALERT_WINDOW_FRAMES}  cd={alert_cooldown_left}",
                    f"State    Warn pred/GT={int(last_infer_warn_state)}/{int(last_infer_gt_warn_state)}  Alert pred/GT={int(last_infer_alert_state)}/{int(last_infer_gt_alert_state)}",
                ]
    ...
    tp_confs = []
    fp_confs = []
    tp_confs_full = []; fp_confs_full = []
    tp_confs_guided = []; fp_confs_guided = []
    tp_confs_verify = []; fp_confs_verify = []
    ...
    # IMPORTANT: Keep CSV schema append-only.
    ...
    detail = {
        ...
        "avg_main_conf_verify_attempts": safe_mean(conf_verify_trigger_main_all),
        "avg_post_conf_verify_attempts_with_box": safe_mean(conf_verify_trigger_post_any),
        "avg_main_conf_verify_pass": safe_mean(conf_verify_pass_main),
        "avg_post_conf_verify_pass": safe_mean(conf_verify_pass_post),
        "avg_main_conf_verify_fail": safe_mean(conf_verify_fail_main),
        "avg_post_conf_verify_fail_with_box": safe_mean(conf_verify_fail_post_any),
        ...
        "avg_conf_tp_verify": safe_mean(tp_confs_verify),
        "avg_conf_fp_verify": safe_mean(fp_confs_verify),
    ...
    detail_csv_path = os.path.join(OUTPUT_ROOT, "benchmark_runs_detailed.csv")
